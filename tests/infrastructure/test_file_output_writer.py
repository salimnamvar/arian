...
class TestFileOutputWriter:
    ...
    def test_writes_content(self, tmp_path: Path) -> None:
        ...
        result = writer.save(str(target), "hello world")
    ...
    def test_empty_content_rejected(self, tmp_path: Path) -> None:
        ...
        writer = FileOutputWriter()
        result = writer.save(str(tmp_path / "out.md"), "")
        assert result.is_success is False
    ...
    def test_creates_parent_directories(self, tmp_path: Path) -> None:
        ...
        writer = FileOutputWriter()
        result = writer.save(str(target), "nested")
        assert result.is_success is True
    ...
    def test_overwrites_existing(self, tmp_path: Path) -> None:
        ...
        writer = FileOutputWriter()
        result = writer.save(str(target), "new")
        assert result.is_success is True
    ...
    def test_no_temp_files_left_after_success(self, tmp_path: Path) -> None:
        ...
        target = tmp_path / "out.md"
        writer = FileOutputWriter()
        result = writer.save(str(target), "clean")
        assert result.is_success is True
